...
struct AgentRuleInstallerTests {
    ...
    func agentRulesPreferMCPWithSelectedFallback() throws {
        ...
        for rules in [agentsRules, sharedRules] {
            ...
            #expect(rules.contains("If `.authsia/workspace.json` exists, use `authsia_workspace_inspect` to list declared `authsia://` refs from that file and managed env files, including env bindings that point at another vault folder"))
            #expect(rules.contains("Keep declared folders as written"))
            #expect(rules.contains("Do not call `authsia_list` to enumerate this workspace folder or any other folder"))
            #expect(rules.contains("`authsia_list` only when there is no workspace config, or for vault metadata that is not already declared there"))
            #expect(!rules.contains("`authsia_list` for scoped CLI-enabled Vault item metadata"))
            #expect(!rules.contains("do not call `authsia_list` to enumerate the vault"))
            ...
            #expect(rules.contains("In CLI fallback mode, if `.authsia/workspace.json` exists, list declared `authsia://` refs from that file or `authsia workspace status`, including env bindings that point at another vault folder"))
            #expect(rules.contains("`authsia list <type> --folder <declared-folder> ...`"))
    ...
    func disabledMCPInstallsCLIOnlyRules() throws {
        ...
        for rules in [agentsRules, sharedRules] {
            ...
            #expect(rules.contains("AUTHSIA_AGENT_PLATFORM=codex"))
            #expect(rules.contains("`authsia exec <type> <query> [options] -- <command> <args>`"))
            #expect(rules.contains("If `.authsia/workspace.json` exists, list declared `authsia://` refs from that file or `authsia workspace status`, including env bindings that point at another vault folder"))
            #expect(rules.contains("Keep those folders as written"))
            #expect(rules.contains("`authsia list <type> --folder <declared-folder> ...`"))
            #expect(rules.contains("Use attributed `authsia list <type> --folder <declared-folder> ...` only when there is no workspace config, or for vault metadata that is not already declared there"))
