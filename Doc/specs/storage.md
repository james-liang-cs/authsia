...
| `~/Library/Application Support/Authsia/agent-command-history.jsonl` | CLI / hooks / Access Center | Redacted agent command-history evidence for Access Center, and the production attribution source. Attribution uses a 5-minute window from `recordedAt`, and a record stays claimable by a new process for its first 60 seconds so an unconsumed record cannot shift later attributions; `contextExpiresAt` (60 minutes) is history retention. |
...
| `~/Library/Application Support/Authsia/agent-command-history.claimed.json` | CLI | `0600` claim sidecar keyed by command-history record id so concurrent `authsia` processes do not reuse one hook record. A claim is taken as a compare-and-set, so only one process may report a record as exact. Claims are dropped once the history file no longer holds the record, or after 60 minutes. |
| `~/Library/Application Support/Authsia/agent-command-history.claimed.lock` | CLI | Empty `0600` advisory-lock sidecar that serializes claim reads and writes across concurrent `authsia` processes. |
...
| `~/Library/Application Support/Authsia/AgentRuntimeContext/lineage.jsonl` | CLI / fallback SubagentStart-Stop hooks / Access Center | Display-only sub-agent start/stop records (`kind: subagent`) with a 24-hour TTL. Never contains prompts or transcripts. The hook scripts only append; the CLI and app merge, prune, and cap the file under the lock. Override with `AUTHSIA_HOOK_LINEAGE_PATH`. |
| `~/Library/Application Support/Authsia/AgentRuntimeContext/lineage.jsonl.lock` | CLI / Access Center | Empty `0600` advisory-lock sidecar that serializes lineage merges and rewrites. |
