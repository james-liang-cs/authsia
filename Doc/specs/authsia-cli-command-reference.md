...
| 2 | Compatible existing Claude settings | parseable `.claude/settings.local.json` with custom content and compatible or absent/`null` containers | `authsia agent init --agent claude-code` | Structurally adds every exact Authsia hook and the outside-sandbox Authsia/Git/SSH command exclusions, removes the obsolete Authsia socket and Mach lookup values, and preserves custom content; absent and `null` containers are safe empty containers |
| 3 | Repeat Claude setup | settings produced or merged by step 1 or 2 | `authsia agent init --agent claude-code` | Idempotent: reports the settings unchanged and does not duplicate hooks or command exclusions |
...
| 5 | Codex rules | N/A | `authsia agent init --agent codex` | Creates or updates `.authsia/agent-rules.md` and `AGENTS.md` with workspace-inspect inventory from `.authsia/workspace.json`, MCP-first execution routing, and attributed workspace/non-workspace CLI fallbacks |
...
hook, generated outside-sandbox command exclusion, exact `~/.authsia/agent.sock`
network value, and legacy `Authsia.Bridge` and `Authsia.SSHAgent` Mach lookup
values across duplicate entries and matchers. If
